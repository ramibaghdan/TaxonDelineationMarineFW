...
#saveRDS(cluster.habitats.relevant, "clusterhabitatsrelevantBranchio.Rdata")

#cluster.habitats.relevant <- readRDS("clusterhabitatsrelevantBranchio.Rdata")
...
#Create second t-test function to retain only freshwater and marine average silhouette indices (relevant data for statistics).

t.test.sil.2 <- function(cluster.habitat.relevant) {
...
#Execute t-test function on data sets.
t.test.results <- lapply(cluster.habitats.relevant, t.test.sil.2)
...
stat.extract <- function(clusterdata, ttestresult) {
  ...
  for(i in 1:length(clusterdata)) {
    ...
    tax.name <- unique(clusterdata[[i]]$order_name)
...
form.table
#Save results.
#saveRDS(form.table, "BranchioTableResults.RData")
...
form.table1 <- readRDS("Data/MalacoTableResults.RData")
...
form.table2 <- readRDS("Data/ActinoTableResults.RData")
...
form.table3 <- readRDS("Data/BranchioTableResults.RData")
...
form.table4 <- readRDS("Data/BivalviaTableResults.RData")
