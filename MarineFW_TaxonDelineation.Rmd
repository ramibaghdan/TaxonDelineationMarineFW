...
library(tidyverse)
#install.packages("msa",repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
library(msa)
...
library(annotate)
#install.packages("factoextra", repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
library(factoextra)
#install.packages("sf",repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
library(sf)
#install.packages("spData",repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
library(spData) 
#install.packages("maptools",repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
library(maptools)
#Load world data for GPS based habitat designations.
data(wrld_simpl)
#devtools::install_github("CNuge/coil", build_vignettes = TRUE)
library(coil)
#install.packages("dendextend")
library(dendextend)
#install.packages("colorspace",repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
library(colorspace)
#install.packages("ggplot2",repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
library(ggplot2)
#install.packages("hrbrthemes",repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
library(hrbrthemes)
#install.packages("ggalt",repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
library(ggalt)
#install.packages("formattable",repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
library(formattable)
#install.packages("rnaturalearth",repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
library("rnaturalearth")
#install.packages("rnaturalearthdata",repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
library("rnaturalearthdata")
#install.packages("rstatix",repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
library(rstatix)
#install.packages("ggpubr",repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
library(ggpubr)
#install.packages("rgeos", ,repos="https://mirror.rcg.sfu.ca/mirror/CRAN/")
library(rgeos)

#Set working directory.
#setwd('~/Desktop/RA_Guelph/Project1/TaxonDelineationMarineFW')

#Set parameters.

#We use the Tamura and Nei 1993 (TN93) distance model for sequence clustering, an effective model for marine COI-5P sequences. For arctic aquatic insect species (Zhou et al., 2009) and most animal species (Hebert et al., 2003), barcode clusters using a 2% threshold resulted in similar estimations to the morphological species concept utilized by taxonomists. Furthermore, divergences at COI greater than 2% tend to be associated with reproductive isolation in freshwater fishes. Therefore, for our main analysis, a 2% clustering threshold was used.
...
#In this section, records for a taxonomic class of interest are downloaded from BOLD. Preliminary filtering of the data set removes low quality records (see below filter.records function for details).
...
dataset <- read_tsv("Data/bold_dataBranchioGlobal.tsv")
...
#4. For this study, records receiving a mixed designation at the species level will receive a final habitat designation of "na" regardless of other designations obtained using habitat/region/exact-site or GPS information.
...
#If a record has no GPS derived habitat designation but a habitat/region/exact-site derived designation (Marine or Freshwater), the record will receive the habitat/region/exact-site designation, and vice-versa.
...
#Obtain distinct rows to focus only on unique BIN, habitat, and counts combinations.
...
#Among the retained BINs, check whether the next taxonomic rank (order) meets the sample size criteria.
...
COI.alignments <- readRDS("Data/COIalignmentsBranchio.Rdata")
barcode_data_coil_filtered_sets <- readRDS("Data/barcode_data_coil_filtered_setBranchio.Rdata")
sample.check <- readRDS("Data/samplecheckBranchio.Rdata")
...
#Initiate empty list to include merged data frames.
...
#saveRDS(cluster.habitats, "clusterhabitatsBranchio.RData")

#Create function to produce colored dendrograms.----

#Color dendrograms according to freshwater and marine.

#Create clustering function again but obtain the dendrogram only.

cluster.function.dend <- function(COI.alignment) {
...
#Execute cluster function on list of sequence alignments (to obtain the dendrograms this time).
clustersdend <- lapply(COI.alignments, cluster.function.dend)
...
color.dendrogram <- function(clusterhabitats, clustersdend, groupnames) {
  ...
  for (i in 1:length(clusterhabitats)){
    ...
    #Sort col_GROUP color palette according to habitats: Turquoise for Marine, Magenta for Freshwater, and Gray for na.
...
coldends <- color.dendrogram(cluster.habitats, clustersdend, groupnames)
...
samplecheck.function <- function(cluster.habitats, sample.check) {
  ...
  #Input: list of clustering results with habitat information and list of sample size criteria for each taxonomic group.
...
#saveRDS(cluster.habitats.relevant, "Data/clusterhabitatsrelevantBranchio.Rdata")

cluster.habitats.relevant <- readRDS("Data/clusterhabitatsrelevantBranchio.Rdata")


#Wilcoxon test function----

#Determine the mean silhouette index value for freshwater and marine clusters and, using an unpaired two-samples Wilcoxon test (or Mann-Whitney test), compare silhouette values across the two groups.

#Create function to run Wilcoxon test analysis.
w.test.sil <- function(cluster.habitat.relevant) {
  
  #Input: clustering results meeting sample size criteria
  #Output: Wilcoxon test results
  ...
  fresh.sils <- uniqfreshsil$mean.sil
  ...
  marine.sils <- uniqmarinesil$mean.sil

  #Run unpaired Wilcoxon test to compare marine and freshwater silhouette indices.
  w.test.result <- wilcox.test(fresh.sils, marine.sils, alternative = "two.sided")
  
  #Obtain statistical test results, freshwater and marine silhouette indices, and recordIDs for freshwater and marine records (to obtain lengths downstream).
  w.test.result.values <- list(w.test.result, fresh.sils, marine.sils, fresh.sil$recordID, marine.sil$recordID)
  
  return(w.test.result.values)
  
}

#Execute Wilcoxon test function on data sets.
w.test.results <- lapply(cluster.habitats.relevant, w.test.sil)

w.test.results2 <- sapply(w.test.results,`[`, 1)

#Obtain average silhouette indices from results for freshwater and marine clusters.
fresh.sils <- sapply(w.test.results, "[[", 2)
marine.sils <- sapply(w.test.results, "[[", 3)
...
fresh.sil <- sapply(w.test.results,"[[", 4)
marine.sil <- sapply(w.test.results,"[[", 5)
...
chr.Actino <- readRDS("Data/clusterhabitatsrelevantActino.Rdata")
chr.Bivalvia <- readRDS("Data/clusterhabitatsrelevantBivalvia.Rdata")
chr.Branchio <- readRDS("Data/clusterhabitatsrelevantBranchio.Rdata")
chr.Malaco <- readRDS("Data/clusterhabitatsrelevantMalaco.Rdata")
...
#Run w.test function to obtain freshwater and marine silhouette values.
w.test.results.violin <- lapply(chr.all, w.test.sil)

#Obtain average silhouette indices from results for freshwater and marine clusters.
fresh.sils.violin <- sapply(w.test.results.violin, "[[", 2)
marine.sils.violin <- sapply(w.test.results.violin, "[[", 3)
...
#Assign group colors (magenta for freshwater and turquoise for marine)
...
#Obtain outliers using the identify_outliers function.

####Check if data is normally distributed (using original data and log/sqrt transformed data). In this case, we find the data to not be normally distributed across the majority of the taxa, so a non-parametric test (unpaired two-sample Wilcoxon test) was used instead of a parametric test (Welch's t test).

#Check range of silhouette values.
range(df.long$Sil.values)

#Transform values using square root function.
#df.long$Sil.values <- sqrt(df.long$Sil.values)
#df.long <- na.omit(df.long)

#Transform values using log function (base e).
#df.long$Sil.values <- log(df.long$Sil.values)
#df.long <- na.omit(df.long)

#It is best to split the data by taxon to obtain taxon-specific outliers for freshwater and marine indices.

dataframes.outlier <- split(df.long, df.long$Taxa)

#Create function to obtain outliers.

outlier.fun <- function(df) {
  
  df.idout <- df %>% 
  group_by(Type) %>% filter(!is.na(Sil.values)) %>%
  identify_outliers(Sil.values)
  
  return(df.idout)
  
}

#Execute outlier function on list of taxa.
df.long.idout <- lapply(dataframes.outlier, outlier.fun)
df.long.idout <- do.call("rbind", df.long.idout)

#Obtain freshwater and marine outliers.
df.long.idout.fresh <- df.long.idout %>% filter(Type == "Freshwater")
df.long.idout.marine <- df.long.idout %>% filter(Type == "Marine")

#Check counts of outliers for each taxon.
table(df.long.idout.fresh$Taxa)
table(df.long.idout.marine$Taxa)

#Include only non-outlier cluster silhouette values.
df.long.no.out <- df.long %>% anti_join(df.long.idout)

#Checking normality by computing Shapiro Wilk test by group.

#Split data frames by taxa.
dataframes.noout <- split(df.long.no.out, df.long.no.out$Taxa)

#Create function to check normality.
normality.fun <- function(df) {
  
  df.norm <- df %>%
  group_by(Type) %>% filter(!is.na(Sil.values)) %>%
  shapiro_test(Sil.values)
  
  df.norm$Taxa <- unique(df$Taxa)
  
  return(df.norm)
  
}

#Shapiro test needs at least 3 samples (Dreissenidae must be excluded)
df.long.norm.no.out <- lapply(dataframes.noout[-8], normality.fun)
df.long.norm.no.out <- do.call("rbind", df.long.norm.no.out)

#Check again including outliers.
df.long.norm <- lapply(dataframes.outlier[-8], normality.fun)
df.long.norm <- do.call("rbind", df.long.norm)

#Re-organize column names
df.long.norm.no.out <- df.long.norm.no.out[,c(5,1,2,3,4)]
df.long.norm <- df.long.norm[,c(5,1,2,3,4)]

#write_csv(df.long.norm.no.out, "normality_no_outliers.csv")
#write_csv(df.long.norm, "normality_with_outliers.csv")
#write_csv(df.long.norm.no.out, "normality_no_outliers_logtr.csv")
#write_csv(df.long.norm, "normality_with_outliers_logtr.csv")
#write_csv(df.long.norm.no.out, "normality_no_outliers_sqrttr.csv")
#write_csv(df.long.norm, "normality_with_outliers_sqrttr.csv")

#Divide data set into 2 for qqplots (visualizing normality) 
dataframes.noout1 <- dataframes.noout[1:9]
dataframes.noout2 <- dataframes.noout[10:18]
dataframes.noout.combined1 <- do.call("rbind", dataframes.noout1)
dataframes.noout.combined2 <- do.call("rbind", dataframes.noout2)

# Draw a qq plot by group (two sets of data for visualization)
ggqqplot(dataframes.noout.combined1, x = "Sil.values", facet.by = c("Type", "Taxa"))

ggqqplot(dataframes.noout.combined2, x = "Sil.values", facet.by = c("Type", "Taxa"))


####Function to combine relevant statistical results----

#Create second Wilcoxon test function to retain only freshwater and marine average silhouette indices (relevant data for statistics).


#Initiate empty list for statistical results.
stat.data <- list()

####IMPORTANT: for taxonomic classes where the datasets retained are at the ORDER level (Branchiopoda), call on the order_name from cluster data to create the tax.name object in the stat.extract function.

stat.extract <- function(clusterdata, wtestresult, fresh.sils, marine.sils) {
  
  #Input: clustering results meeting sample size criteria and Wilcoxon test results.
  #Output: data frame of statistical results.
  
  #Iterate over clustering results.
  for(i in 1:length(clusterdata)) {
    
    #Obtain name of taxonomic group, number of unique bins, freshwater mean silhouette value, marine mean silhouette value, and w-test p-value. 
    tax.name <- unique(clusterdata[[i]]$family_name)
    uniqbins <- length(unique(clusterdata[[i]]$bin_uri))

    wtest.fresh.est <- mean(fresh.sils[[i]])
    wtest.marine.est <- mean(marine.sils[[i]])
    wtest.pval <- wtestresult[[i]]$p.value
  
    #Place results into data frame.
    stat.data[[i]] <- data.frame("Taxon" = tax.name, "No.of.bins" = uniqbins, "Fresh.av.sil" = wtest.fresh.est, "Marine.av.sil" = wtest.marine.est, "w.test.p.values" = wtest.pval)
  }
...
#statoutputs <- stat.extract(cluster.habitats.relevant, w.test.results)
statoutputs <- stat.extract(cluster.habitats.relevant, w.test.results2, fresh.sils, marine.sils)
...
form.table <- formattable(statoutputs, list(`Taxon` = formatter(
  ...
  `Marine.av.sil`= color_tile(customGreen0, customGreen), `Difference` = difference_formatter, `w.test.p.values` = pvalue_formatter))
...
form.table$w.test.p.values <- round(form.table$w.test.p.values, 3)

form.table


#Save results.
#saveRDS(form.table, "W_BranchioTableResults.RData")
#saveRDS(form.table, "W_MalacoTableResults.RData")
#saveRDS(form.table, "W_ActinoTableResults.RData")
#saveRDS(form.table, "W_BivalviaTableResults.RData")
...
form.table1 <- readRDS("W_MalacoTableResultsComp.RData")
...
form.table2 <- readRDS("W_ActinoTableResultsComp.RData")
...
form.table3 <- readRDS("W_BranchioTableResultsComp.RData")
...
form.table4 <- readRDS("W_BivalviaTableResultsComp.RData")
...
form.table1 <- readRDS("W_MalacoTableResults.RData")
form.table2 <- readRDS("W_ActinoTableResults.RData")
form.table3 <- readRDS("W_BranchioTableResults.RData")
form.table4 <- readRDS("W_BivalviaTableResults.RData")
...
#resultstable$Fresh.av.sil <- sqrt(resultstable$Fresh.av.sil)
#resultstable$Marine.av.sil <- sqrt(resultstable$Marine.av.sil)

#Obtain outliers in overall freshwater and marine silhouette values across taxa.
master.out <- rbind(identify_outliers(resultstable, variable = "Fresh.av.sil"),identify_outliers(resultstable, variable = "Marine.av.sil"))

#Exclude outliers from results table.
resultstable.noout <- anti_join(resultstable, master.out)

#Check if the data is normally distributed with an without outliers.
master.normality <- rbind(shapiro_test(resultstable$Fresh.av.sil),shapiro_test(resultstable$Marine.av.sil))

master.normality.noout <- rbind(shapiro_test(resultstable.noout$Fresh.av.sil),shapiro_test(resultstable.noout$Marine.av.sil))

#Add habitat type label for clarification.
master.normality$variable <- c("Average freshwater values", "Average marine values")
master.normality.noout$variable <- c("Average freshwater values", "Average marine values")

#Save normality test results.
#write_csv(master.normality, "masternormality_with_out.csv")

#Execute a paired t-test to determine whether there is a significant difference between overall marine and freshwater silhouette indices.
master.t.test.result <- t.test(resultstable.noout$Fresh.av.sil, resultstable.noout$Marine.av.sil, paired = TRUE)
master.t.test.result

#Find mean freshwater and silhouette overall.
mean(resultstable.noout$Fresh.av.sil)
mean(resultstable.noout$Marine.av.sil)

#Adjusted p-values according to Benjamini & Hochberg method for taxon-specific Wilcoxon tests.
adjusted.p <- p.adjust(form.tableall$w.test.p.values, method = "BH")
adjusted.p
...
#In this section, silhouette plots were obtained for the Mysidae family.
...
names(form.tableall)
form.tableall$group1 <- c(rep("Actinopterygii", 7),"Bivalvia","Branchiopoda",rep("Malacostraca", 9))
records.sils.fresh <- form.tableall[,c("Fresh.records","Fresh.av.sil", "group1")]
records.sils.marine <- form.tableall[,c("Marine.records","Marine.av.sil", "group1")]
...
#Add habitat type to each row.
replength <- length(form.tableall$Taxon)
groupvector <- c(rep("Freshwater", replength), rep("Marine", replength))

records.sils$habtype <- groupvector

#Execute linear regression model. Average silhoutte values (dependent variable) vs Number of records (independent variable) for each habitat type.
model.1 <- lm(av.sil[1:18] ~ record.number[1:18], records.sils)
model.2 <- lm(av.sil[19:36] ~ record.number[19:36], records.sils)
...
dataset.actino <- readRDS("Data/datasetslistActino.RData")
dataset.branchio <- readRDS("Data/datasetslistBranchio.RData")
dataset.malaco <- readRDS("Data/datasetslistMalaco.RData")
