...
cluster.habitats.relevant <- readRDS("Data/clusterhabitatsrelevantBivalvia.Rdata")
...
w.test.sil.2 <- function(cluster.habitat.relevant) {
  ...
  # outlier.fun <- function(df) {
  # 
  # df.idout <- df %>% group_by(habitat.final.y) %>%
  # identify_outliers(mean.sil)
  # 
  # return(df.idout)
  # 
  # }
  # 
  # fresh.sils.out <- outlier.fun(uniqfreshsil)
  # marine.sils.out <- outlier.fun(uniqmarinesil)
  # 
  # fresh.no.out <- uniqfreshsil %>% anti_join(fresh.sils.out)
  # marine.no.out <-uniqmarinesil %>% anti_join(marine.sils.out)
  # 
  # fresh.sils <- fresh.no.out$mean.sil
  # marine.sils <- marine.no.out$mean.sil
  # 
  # clusterdata.no.out <- rbind(fresh.no.out, marine.no.out)
...
#Execute w-test function on data sets.
w.test.results <- lapply(cluster.habitats.relevant, w.test.sil.2)
...
statoutputs$Fresh.clust <- length(fresh.sils[[1]])
...
statoutputs$Marine.clust <- length(marine.sils[[1]])
...
#Use below code for Branchiopoda and Bivalvia group.
statoutputs$Fresh.records <- length(fresh.sil[[1]])
statoutputs$Marine.records <- length(marine.sil[[1]])
...
ggqqplot(resultstable$Fresh.av.sil)
