...
## ----Data download and preliminary filtering, results = 'hide', message=FALSE, warning=FALSE-----------------------------------

#Download records for taxonomic classes of interest from BOLD
...
## ----BIN-level habitat assignments according to sample size critera, message=FALSE, results = 'hide', warning=FALSE------------
...
## ----DNA sequence quality check and sequence alignment, results = 'hide', message=FALSE, warning=FALSE-------------------------
...
COI.alignments <- readRDS("COIalignmentsBranchio.Rdata")
barcode_data_coil_filtered_sets <- readRDS("barcode_data_coil_filtered_setBranchio.Rdata")
sample.check <- readRDS("samplecheckBranchio.Rdata")
...
#plot(clusters$Onychopoda[2][[1]])
clustersCOI <- lapply(COI.alignments, cluster.function)
...
color.dendrogram <- function(clusterhabitats, clustersCOI, groupnames) {
...
coldends <- color.dendrogram(cluster.habitats, clustersCOI, groupnames)
...
ggplot(df.long, aes(x=factor(Taxa, levels=unique(Taxa)), y=Sil.values, color = Type)) + geom_violin() + theme(axis.text.x = element_text(angle = 60, vjust = 0.9, hjust=0.9, colour = label.colors), plot.title = element_text(hjust = 0.5)) + xlab("Taxa") + ylab("Silhouette Values") + ggtitle(" Distribution of Silhouette Values") + scale_colour_manual(values=group.colors) + facet_grid(. ~ Type, scales = "free_y")
...
form.table1 <- readRDS("MalacoCompTableResults.RData")
...
form.table2 <- readRDS("ActinoCompTableResults.RData")
...
form.table3 <- readRDS("BranchioCompTableResults.RData")
...
form.table4 <- readRDS("BivalviaCompTableResults.RData")
...
master.t.test.result <- t.test(resultstable$Fresh.av.sil, resultstable$Marine.av.sil)
master.t.test.result

#Adjusted p-values according to Benjamini & Hochberg method.
